...
class AMIChatbot {
  constructor() {
    ...
    this.messageHistory = [];
    this.apiKey = window.careerMentorApp && typeof window.careerMentorApp.getOpenAiApiKey === 'function' ? window.careerMentorApp.getOpenAiApiKey() : null; // Get OpenAI API key
    // initChatbot() call removed, will be handled by DOMContentLoaded or toggleVisibility
  }
  ...
  // initChatbot() method removed as its functionality is consolidated into init() and createUI()
  // The old UI created by initChatbot was simpler and has been superseded by the one in createUI.
  ...
  sendMessage() {
    ...
    // Using async/await with Gemini API
    this.processMessage(userMessage.content)
      ...
      .catch(error => {
        ...
        const fallbackMessage = {
          sender: 'ami',
          content: `I'm sorry, I encountered an issue processing your request. How else can I assist you with your career journey?`
        };
        this.messages.push(fallbackMessage);
        this.addMessageToDOM(fallbackMessage);
      });
  }
  ...
  async processMessage(userMessage) {
    ...
    // Try to use Gemini API for more intelligent responses
    if (this.apiKey) { // Check for OpenAI key now
        try {
            const openAiResponse = await this.getOpenAiResponse(userMessage);
            if (openAiResponse) {
                ...
                // Re-attach event listeners to any quick action buttons in the response
                this.setupQuickActionButtons();
                return;
            }
        } catch (error) {
            console.error('Error getting response from OpenAI:', error);
            // Continue with fallback responses if Gemini fails
        }
    } else {
        console.warn('OpenAI API key not configured. Falling back to rule-based responses.');
    }
    
    // Fallback to rule-based responses if Gemini API key is missing or API call fails
    // Resume analysis features
      // ... (rest of the existing fallback logic)
        // Continue with fallback responses if OpenAI fails
    }
          ...
          window.careerMentorApp.analyzeResume(userMessage)
            .then(analysis => {
              ...
              const analysisResponse = `
                <div class="p-3 border-l-4 border-indigo-500 mb-2 bg-indigo-900/20 rounded">
                  <p class="font-semibold mb-2">Resume Analysis Results:</p>
                  <div class="mb-1"><span class="text-indigo-400 font-medium">Match Score:</span> ${analysis.matchScore}%</div>
                  <div class="mb-1"><span class="text-indigo-400 font-medium">Technical Skills:</span> ${skillsList || 'None detected'}</div>
                  <div class="mb-1"><span class="text-indigo-400 font-medium">Key Strengths:</span> ${strengthsList || 'None detected'}</div>
                  ${gapsList ? `<div class="mb-1"><span class="text-indigo-400 font-medium">Consider adding:</span> ${gapsList}</div>` : ''}
                </div>
                <p>Would you like more detailed feedback or job recommendations based on your resume?</p>
                <div class="mt-2 flex space-x-2">
                  <button class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs px-3 py-1 rounded-full transition-all ami-quick-action" data-query="Show me detailed feedback">Detailed feedback</button>
                  <button class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs px-3 py-1 rounded-full transition-all ami-quick-action" data-query="Show job recommendations">Job recommendations</button>
                </div>
              `;
              ...
              // Re-attach event listeners to the dynamically added quick action buttons
              this.setupQuickActionButtons();
            })
            .catch(error => {
              this.removeTypingIndicator();
      ...
      if (hasCareerFeatures) {
        ...
        window.careerMentorApp.getJobRecommendations()
          .then(jobs => {
            ...
            if (jobs.length > 0) {
              // Format job recommendations with improved styling
              let jobListHTML = '<div class="space-y-2 my-3">';
              
              jobs.slice(0, 3).forEach(job => {
                const matchColor = job.matchScore >= 90 ? 'text-green-400' : (job.matchScore >= 70 ? 'text-yellow-400' : 'text-gray-400');
                
                jobListHTML += `
                  <div class="p-3 rounded-lg bg-gray-800/70 border border-indigo-500/30 hover:border-indigo-400/50 transition-all duration-200">
                    <div class="flex justify-between items-start">
                      <div class="font-medium text-white">${job.title}</div>
                      <div class="${matchColor} text-sm font-semibold">${job.matchScore}%</div>
                    </div>
                    <div class="text-gray-300 text-sm">${job.company}</div>
                    <div class="flex items-center justify-between mt-2">
                      <div class="text-gray-400 text-xs">${job.location}</div>
                      <button class="text-xs text-indigo-400 hover:text-indigo-300 transition-colors ami-quick-action" data-query="Show details of ${job.title} at ${job.company}">View details</button>
                    </div>
                  </div>
                `;
              });
              ...
              const jobResponse = `
                ...
                <div class="mt-2 flex space-x-2">
                  <button class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs px-3 py-1 rounded-full transition-all ami-quick-action" data-query="Show more job recommendations">More jobs</button>
                  <button class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs px-3 py-1 rounded-full transition-all ami-quick-action" data-query="Tips for applying to jobs">Application tips</button>
                </div>
              `;
...
Visit the Jobs section of the dashboard to explore opportunities matching your profile.`;
    ...
    // Interview preparation features
      ...
      if (hasCareerFeatures && 
        ...
        // TODO: Enhance to ask user for experience level, skills, and interview type
        const experienceLevel = "mid-level"; // Placeholder
        const skills = [jobTitle.includes("Data") ? "Python" : "JavaScript", "Problem Solving"]; // Placeholder
        const interviewType = "technical and behavioral"; // Placeholder

        window.careerMentorApp.getInterviewQuestions(jobTitle, company, experienceLevel, skills, interviewType)
          .then(questions => {
            this.removeTypingIndicator();
...
Which area would you like to explore first?`;
  ...
  setupQuickActionButtons() {
    // Initialize any quick action buttons
    setTimeout(() => {
      document.querySelectorAll('.ami-quick-action').forEach(button => {
        ...
        newButton.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          const query = newButton.dataset.query;
          if (this.inputField) {
            this.inputField.value = query;
            this.sendMessage();
          }
        ...
        newButton.onmouseover = () => {
          newButton.style.boxShadow = '0 0 8px rgba(99, 102, 241, 0.5)';
  ...
  async getOpenAiResponse(userMessage) {
    try {
      // Create a context-aware prompt that includes user information and previous messages
      let contextMessages = this.messages.slice(-5); // Get last 5 messages for context
      let contextText = contextMessages.map(msg => 
        `${msg.sender === 'user' ? 'User' : 'AMI'}: ${msg.sender === 'user' ? msg.content : this.stripHTML(msg.content)}`
      ).join('\n');
      
      const prompt = `You are AMI (AI Mentor Interface), an intelligent career assistant chatbot. 
      You help users with career development, job searching, resume building, interview preparation, and skill development.
      ...
      Respond in a helpful, friendly, and professional manner. Focus on career-related guidance.
      If the user asks about features of the Career Mentor application, explain them clearly.
      
      Key features you can discuss:
      - Resume Analysis: Analyzes resumes and provides feedback
      - Job Market Predictions: Shows job trends and opportunities
      - Learning Paths: Recommends courses and skills to develop
      - Interview Preparation: Provides practice questions and feedback
      - Smart Autofill: Helps fill out job applications efficiently
      - Work Assistant: Helps organize tasks and deadlines
      
      Format your response with HTML for rich formatting. You can use:
      - <p> for paragraphs
      - <strong> for emphasis
      - <ul> and <li> for lists
      - <div> with classes for styled sections
      
      Keep your response concise but informative.`;
      
      this.apiKey = window.careerMentorApp && typeof window.careerMentorApp.getOpenAiApiKey === 'function' ? window.careerMentorApp.getOpenAiApiKey() : null;
    if (!this.apiKey) {
      console.error('OpenAI API key is missing. Please configure it in initialize.js');
      return null; // Fallback to rule-based if key is missing
    }
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-3.5-turbo',
          messages: [
            {role: 'system', content: 'You are AMI (AI Mentor Interface), an intelligent career assistant chatbot. You help users with career development, job searching, resume building, interview preparation, and skill development. Format your response with HTML for rich formatting. You can use: <p> for paragraphs, <strong> for emphasis, <ul> and <li> for lists, <div> with classes for styled sections. Keep your response concise but informative.'},
            ...this.messages.slice(-5).map(msg => ({role: msg.sender === 'user' ? 'user' : 'assistant', content: msg.sender === 'user' ? msg.content : this.stripHTML(msg.content)})),
      ...
      return null; // Return null to fall back to rule-based responses
    }
  }
  ...
  // Removed addEventListeners(), toggleChat(), minimizeChat(), closeChat(), addWelcomeMessage(), addBotMessage(), scrollToBottom(), escapeHTML()
  // These methods were part of an older/alternative UI structure.
  // The current UI is created by createUI() and its events are handled by setupEventListeners().
  // Welcome messages are handled by addWelcomeMessages().
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Call init() to set up the main UI and event listeners after DOM is loaded.
  // The init() method itself ensures it only runs once.
